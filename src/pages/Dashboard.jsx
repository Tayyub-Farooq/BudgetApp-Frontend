...
export default function Dashboard() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [eRes, sRes, oRes, cardsRes, alertsRes] = await Promise.all([
      ...
      const mappedExpenses = (eRes.expenses || []).map((x) => ({
        id: x._id || x.id,
        category: x.category,
        amount: x.amount,
        note: x.note,
      ...
      setOverview(oRes || { total: 0, budget: 0, remaining: 0, percentage: 0, alert: null });
  ...
  async function handleSaveSuccess() {
    await load(); // reload data after saving an edited expense
  }
          ...
          <div className="max-w-6xl mx-auto mt-4 px-4">
            <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-lg px-4 py-3 text-sm flex items-center gap-2">
              ...
              <div>
                ...
                {cardAlerts
                  .map((c) => `${c.cardName} (•••• ${c.cardNumberLast4})`)
                  .join(", ")}
              </div>
            </div>
          </div>
            ...
            {overview.budget > 0 ? (
              <div className="mt-4">
                <div className="flex justify-between text-xs text-slate-500 mb-1">
                  ...
                  <span
                    className={
                      overview.remaining < 0 ? "text-red-600 font-medium" : ""
                    }
                  >
        ...
        <section id="add" className="bg-white border rounded-lg">
          ...
          <form onSubmit={addExpense} className="p-4 grid gap-3 md:grid-cols-4">
            <select name="category" required className="border rounded px-3 py-2">
              ...
              {["Food", "Transport", "Bills", "Shopping", "Health", "Entertainment", "Other"].map(
                (c) => (
